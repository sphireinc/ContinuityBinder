...
describe('encrypted backup', () => {
  it('copies encrypted envelopes without exposing their contents', async () => {
    ...
    const database = { vaultMeta: { get: async () => ({ key: 'header', header }) }, encryptedRecords: { toArray: async () => [envelope] }, migrationMeta: { toArray: async () => [{ key: 'schema', value: '1' }] } } as never;
  ...
  it('rejects a modified encrypted backup', async () => {
    const database = { vaultMeta: { get: async () => ({ key: 'header', header: { vaultFormatVersion: 1, kdf: 'PBKDF2-HMAC-SHA-256', iterations: 310000, salt: 'salt', wrapIv: 'iv', wrappedDek: 'wrapped', createdAt: 'now', updatedAt: 'now' } }) }, encryptedRecords: { toArray: async () => [] }, migrationMeta: { toArray: async () => [] } } as never;
