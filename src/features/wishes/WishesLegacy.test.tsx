...
describe('wishes and legacy', () => {
  it('shows the jurisdiction boundary and plain-text letter guidance', () => {
    render(
      <MemoryRouter>
        <WishesLegacy database={null} dek={{} as CryptoKey} />
      </MemoryRouter>,
    );
    expect(
      screen.getByText(/Whether they are legally binding/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/arbitrary HTML is never rendered/i),
    ).toBeInTheDocument();
    expect(screen.getByLabelText('Clothing')).toBeInTheDocument();
    expect(
      screen.getByLabelText('Organ-donation note/reference'),
    ).toBeInTheDocument();
  });

  it('supports sealed letters and per-adult arrangements', () => {
    const wishes = wishesSchema.parse({
      id: 'w',
      schemaVersion: 1,
      createdAt: 'now',
      updatedAt: 'now',
      personId: 'p',
      arrangement: 'cremation',
    });
    const letter = personalLetterSchema.parse({
      id: 'l',
      schemaVersion: 1,
      createdAt: 'now',
      updatedAt: 'now',
