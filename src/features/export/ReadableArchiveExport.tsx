...
export function ReadableArchiveExport({
  ...
  useEffect(() => {
    ...
    void Promise.all([
    ...
    ]).then(([benefits, people, letters, plans, certificates, estateTasks, claims, accountActions, licenses, militaryRecords, foreignRecords, travelRecords, loyaltyRecords, outstandingRecords, warrantyRecords, storageRecords, collectionRecords, doNotThrowAwayRecords, noValueRecords, personalStoryRecords, recipeRecords, lifeStoryRecords, photoVideoArchiveRecords, backupStorageRecords, hardwareSecurityKeyRecords, cryptoDigitalAssetRecords, aiOnlineServiceRecords]) => {
      const names = new Map(
        people.map((person) => [
          person.id,
      ...
      const recipesTraditionsContent = recipeRecords.filter((record) => record.includeInReadableExport).flatMap((record) => [
        ...
        `${t('family_recipes_traditions.originPerson', { ns: 'v2' })}: ${record.originPersonId || t('family_recipes_traditions.unknown', { ns: 'v2' })} | ${t('family_recipes_traditions.whenObserved', { ns: 'v2' })}: ${record.whenObserved}`,
      ...
      const photoVideoArchiveContent = photoVideoArchiveRecords.filter((record) => record.includeInReadableExport).flatMap((record) => [
        ...
        `${t('photo_video_archive_map.collectionSource', { ns: 'v2' })}: ${record.collectionSource} | ${t('photo_video_archive_map.owner', { ns: 'v2' })}: ${record.ownerId}`,
      ...
      const backupStorageContent = backupStorageRecords.filter((record) => record.includeInReadableExport).flatMap((record) => ['Continuity Binder', 'DIGITAL & INTELLECTUAL PROPERTY', t('backup_storage_map.title', { ns: 'v2' }), `${t('prepared', { ns: 'rendering' })}: ${new Date().toLocaleDateString()}`, `${t('backup_storage_map.systemCollection', { ns: 'v2' })}: ${record.systemCollection} | ${t('backup_storage_map.primaryLocation', { ns: 'v2' })}: ${record.primaryLocation}`, `${t('backup_storage_map.backupMethod', { ns: 'v2' })}: ${record.backupMethod} | ${t('backup_storage_map.backupLocation', { ns: 'v2' })}: ${record.backupLocation} | ${t('backup_storage_map.frequency', { ns: 'v2' })}: ${record.frequency}`, `${t('backup_storage_map.encryptionNote', { ns: 'v2' })}: ${record.encryptionNote} | ${t('backup_storage_map.recoveryInstructionLocation', { ns: 'v2' })}: ${record.recoveryInstructionLocation}`, `${t('backup_storage_map.responsiblePerson', { ns: 'v2' })}: ${record.responsiblePersonId} | ${t('backup_storage_map.technicalContact', { ns: 'v2' })}: ${record.technicalContactId}`, `- [ ] ${t('backup_storage_map.located', { ns: 'v2' })}   - [ ] ${t('backup_storage_map.recoveryTested', { ns: 'v2' })}   - [ ] ${t('backup_storage_map.ownershipTransferred', { ns: 'v2' })}`, `${t('backup_storage_map.date', { ns: 'v2' })}: ____ / ____ / ______    ${t('backup_storage_map.initials', { ns: 'v2' })}: __________    ${t('backup_storage_map.reference', { ns: 'v2' })}: __________`, `${t('backup_storage_map.notes', { ns: 'v2' })}: ________________________________________________________________`, '_______________________________________________________________']);
      ...
      const hardwareSecurityKeysContent = hardwareSecurityKeyRecords.filter((record) => record.includeInReadableExport).flatMap((record) => ['Continuity Binder', 'DIGITAL & INTELLECTUAL PROPERTY', t('hardware_security_key_inventory.title', { ns: 'v2' }), `${t('prepared', { ns: 'rendering' })}: ${new Date().toLocaleDateString()}`, `${t('hardware_security_key_inventory.keyLabel', { ns: 'v2' })}: ${record.keyLabel} | ${t('hardware_security_key_inventory.owner', { ns: 'v2' })}: ${record.ownerId}`, `${t('hardware_security_key_inventory.makeModel', { ns: 'v2' })}: ${record.makeModel} | ${t('hardware_security_key_inventory.physicalLocation', { ns: 'v2' })}: ${record.physicalLocation} | ${t('hardware_security_key_inventory.backupKeyLocation', { ns: 'v2' })}: ${record.backupKeyLocation}`, `${t('hardware_security_key_inventory.associatedServiceCategories', { ns: 'v2' })}: ${record.associatedServiceCategories} | ${t('hardware_security_key_inventory.recoveryInstructionLocation', { ns: 'v2' })}: ${record.recoveryInstructionLocation}`, `- [ ] ${t('hardware_security_key_inventory.located', { ns: 'v2' })}   - [ ] ${t('hardware_security_key_inventory.backupLocated', { ns: 'v2' })}   - [ ] ${t('hardware_security_key_inventory.servicesReviewed', { ns: 'v2' })}`, `${t('hardware_security_key_inventory.date', { ns: 'v2' })}: ____ / ____ / ______    ${t('hardware_security_key_inventory.initials', { ns: 'v2' })}: __________    ${t('hardware_security_key_inventory.reference', { ns: 'v2' })}: __________`, `${t('hardware_security_key_inventory.notes', { ns: 'v2' })}: ________________________________________________________________`, '_______________________________________________________________']);
