...
export function ReadableArchiveExport({
  ...
  useEffect(() => {
    ...
    ]).then(([benefits, people, letters, plans, certificates, estateTasks, claims, accountActions, licenses, militaryRecords, foreignRecords, travelRecords, loyaltyRecords, outstandingRecords, warrantyRecords, storageRecords, collectionRecords, doNotThrowAwayRecords, noValueRecords, personalStoryRecords, recipeRecords, lifeStoryRecords, photoVideoArchiveRecords, backupStorageRecords]) => {
      const names = new Map(
        people.map((person) => [
          person.id,
