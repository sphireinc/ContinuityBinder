...
export const propertyAssetSchema = z.object({
  ...base,
  kind: z.enum(['realEstate', 'vehicle', 'valuable']),
  label: z.string(),
  ...
  vinDisplayPolicy: z.enum(['full', 'last4', 'hidden']).optional(),
  titleDoc: z.string().optional(),
...
type Asset = z.infer<typeof propertyAssetSchema>;
type Person = { id: string; name: string };
const kinds = ['realEstate', 'vehicle', 'valuable'] as const;
const categories = [
  'jewelry',
  'art',
  'collectibles',
...
export function PropertyAssets({
  ...
  const load = () => {
    ...
    ]).then(([nextAssets, nextPeople, nextAddresses]) => {
  ...
  const save = async (event: FormEvent) => {
    ...
    if (!database || !form.label.trim()) return;
    ...
    await createEncryptedRepository(
      ...
      mortgage: form.mortgage.trim() || undefined,
      tax: form.tax.trim() || undefined,
      ...
      maintenance: form.maintenance.trim() || undefined,
      vin: form.vin.trim() || undefined,
      vinDisplayPolicy: form.vin.trim()
        ? (form.vinDisplayPolicy as Asset['vinDisplayPolicy'])
        : undefined,
      titleDoc: form.titleDoc.trim() || undefined,
      registration: form.registration.trim() || undefined,
      lien: form.lien.trim() || undefined,
      spareKey: form.spareKey.trim() || undefined,
      service: form.service.trim() || undefined,
      category: form.category,
      intendedRecipient: form.intendedRecipient.trim() || undefined,
  ...
  return (
    <section className="section-page">
      ...
      <p className="security-clarification">{t('noImages')}</p>
      <div className="section-card">
        ...
        <form onSubmit={(event) => void save(event)}>
          ...
          />
          <SelectField
            label={t('canonicalAddress', {
              defaultValue: 'Canonical address (optional)',
            })}
          ...
          <TextField
            ...
            value={form.mortgage}
            onChange={(event) =>
              setForm({ ...form, mortgage: event.target.value })
            }
          />
          <TextField
            label={t('tax')}
          ...
          <TextField
            label={t('hoa')}
            value={form.hoa}
            onChange={(event) => setForm({ ...form, hoa: event.target.value })}
          />
          <TextField
            label={t('utilities')}
            value={form.utilities}
            onChange={(event) =>
              setForm({ ...form, utilities: event.target.value })
            }
          />
          <TextArea
            label={t('security')}
          ...
          <TextField
            ...
            onChange={(event) =>
              setForm({ ...form, maintenance: event.target.value })
            }
          />
          <TextField
            label={t('vin')}
          ...
          <SelectField
            label={t('vin')}
            value={form.vinDisplayPolicy}
            onChange={(event) =>
              setForm({ ...form, vinDisplayPolicy: event.target.value })
            }
            options={[
              { value: 'full', label: 'Full' },
              { value: 'last4', label: 'Last four' },
          ...
          <TextField
            ...
            value={form.titleDoc}
            onChange={(event) =>
              setForm({ ...form, titleDoc: event.target.value })
            }
          />
          <TextField
          ...
          <TextField
            label={t('lien')}
            value={form.lien}
            onChange={(event) => setForm({ ...form, lien: event.target.value })}
          />
          <TextField
            label={t('spareKey')}
            value={form.spareKey}
            onChange={(event) =>
              setForm({ ...form, spareKey: event.target.value })
            }
          />
          <TextField
            label={t('service')}
          ...
          />
          <TextField
            label={t('intendedRecipient')}
            value={form.intendedRecipient}
            onChange={(event) =>
              setForm({ ...form, intendedRecipient: event.target.value })
            }
            helpText={t('recipientNote')}
          />
          <TextArea
            label={t('notes')}
            value={form.notes}
