...
describe('tax and record retention', () => {
  it('shows the last-three-years prompt and organizational retention boundary', () => {
    render(
      <MemoryRouter>
        <TaxRecords database={null} dek={{} as CryptoKey} />
      </MemoryRouter>,
    );
    expect(
      screen.getByText(/last three years of filed returns/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/not jurisdiction-specific retention advice/i),
    ).toBeInTheDocument();
    expect(screen.getByLabelText('Taxpayer(s)')).toHaveAttribute('multiple');
  });

  it('keeps tax-year and retention records referenceable and structured', () => {
    const tax = taxReturnSchema.parse({
      id: 't',
      schemaVersion: 1,
      createdAt: 'now',
      updatedAt: 'now',
